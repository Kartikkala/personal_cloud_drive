...
import { newInactiveDownload } from '../authentication/utils/userInfoUtils.mjs'
import ufs from 'url-file-size'


export class Aria2Helper extends EventEmitter{
    constructor(aria2Client, statusUpdateInterval){
        if(!aria2Client)
        {
            throw new Error("No aria2 client passed to helper service")
        }
        super()
        this._aria2c = aria2Client
        this.userDownloads = {}
        this.downloadStatusUpdateDuration = statusUpdateInterval*1000 || 2000
    ...
    addStatusUpdateEventEmitter(userId)
    {
        const statusUpdateInterval = setInterval(async ()=>{
            const downloads = this.getUserDownloads(userId)
            const downloadStatus = await this.getUserDownloadStatus(downloads, ["totalLength", "completedLength", "downloadSpeed", "status", "gid", "files"])
            for(let download in downloadStatus)
            {
                if(downloadStatus[download].status === "complete" || downloadStatus[download].status === "error")
                {
                    ...
                    this.removeUserDownload(userId, guid)
                    newInactiveDownload(userId, guid)
                }
            }
            this.emit('statusUpdate_'+userId.toString() ,downloadStatus)
    ...
    async downloadWithURI(uri, user, downloadPath){
        let guid = undefined
        const fileSize = await ufs(uri[0])
        if(fileSize < user.availableSpace)
        {
            guid = await this._aria2c.call("addUri", uri, {dir: downloadPath})
            this.addUserDownload(user._id, guid)
            this.addStatusUpdateEventEmitter(user._id)
